...
### 개인 의견
1. 함수안에서 함수를 선언할 수 없음
2. 기본 타입 int가 아닌 래퍼 클래스 Integer를 사용해야 함. Java의 컬렉션은 객체만을 다루기 때문에 기본 타입 int는 사용할 수 없고, 대신 Integer를 사용해야 됨.
3. 처음에 가로, 세로로만 증가시켜 -1인지 확인하여 조금 헤맴. 가로x세로 전체 칸을 체크해야함.
4. js 보다 코드가 전체적으로 지저분함..(내 실력탓일지도)
