-- 코드를 입력하세요
SELECT T1.REST_ID, T1.REST_NAME, T1.FOOD_TYPE, T1.FAVORITES, T1.ADDRESS
     , ROUND(AVG(T2.REVIEW_SCORE),2) AS SCORE
  FROM REST_INFO T1
 INNER JOIN REST_REVIEW T2
    ON T2.REST_ID = T1.REST_ID
 # WHERE SUBSTR(T1.ADDRESS,1,5) = '서울특별시'
 WHERE T1.ADDRESS LIKE '서울%'
 GROUP BY T1.REST_ID, T1.REST_NAME, T1.FOOD_TYPE, T1.FAVORITES, T1.ADDRESS
 ORDER BY AVG(T2.REVIEW_SCORE) DESC, T1.FAVORITES DESC